...
- For update_inventory_flag: the field is "Inventory". "before" is the current value from the sheet (one of: "in_stock", "low_stock", "out_of_stock", "discontinued", "pre_order"). "after" is the desired flag value (must be one of the same five values). These are text flags, NOT numeric quantities. If the user requests a numeric inventory count (e.g., "400 units"), map it to the closest flag: large counts → "in_stock", small counts → "low_stock", zero → "out_of_stock".
